...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    import tiktoken  # import BÊN TRONG hàm
    try:
        enc = tiktoken.encoding_for_model(model)
        return len(enc.encode(text))
    except Exception:
        # fallback ước lượng thô: trung bình 1 token ≈ 4 ký tự
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        history = history[-6:]  # Giữ lại 3 lượt gần nhất (6 message)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        row = f"{prompt:<40} | {gpt4o_response:<40} | {mini_response:<40} | {gpt4o_latency:<15} | {mini_latency:<15}"
